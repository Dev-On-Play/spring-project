...
public record CommentResponse(long id, long mogako_id, CommentMember member, ChildComments childComments,
                              String contents, LocalDateTime created_date) {

    //user entity 수정 필요
    record CommentMember(long id, String nickname, String profile) {
        public static CommentMember from(Member member) {
            return new CommentMember(member.getId(), member.getNickname(), "profileurl");
        }
    }

    record ChildComments(List<ChildComment> childList) {
    ...
    record ChildComment(long id, long mogako_id, CommentMember member, Long parents_id, String contents,
